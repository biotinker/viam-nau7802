...
var Model = resource.NewModel("biotinker", "sensor", "nau7802")
...
type Config struct {
	...
	Zero    int     `json:"zero,omitempty"` // raw val of what should be considered "zero"
	Kg      float64 `json:"kg,omitempty"`   // raw val of what should be considered 1 kg of weight
}
...
func newSensor(
	...
	s := &nau7802{
		...
		addr:              byte(addr),
		samples:           samples,
		zeroOffset:        attr.Zero,
		calibrationFactor: attr.Kg,
	}
...
// getAverage returns the average of a given number of readings
func (s *nau7802) getAverage(ctx context.Context, samplesToTake int) (int, error) {
	var total int64
	var validReadings int

	timeout := time.Duration(150*samplesToTake) * time.Millisecond
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()

	for validReadings < samplesToTake {
		select {
		case <-ctx.Done():
			return 0, fmt.Errorf("timed out reading data from nau7802: %w", ctx.Err())
		default:
			availBit, err := s.available(ctx)
			if err != nil {
				return 0, fmt.Errorf("error checking availability: %w", err)
			}
			if availBit {
				reading, err := s.getReading(ctx)
				if err != nil {
					return 0, fmt.Errorf("error getting reading: %w", err)
				}
				total += int64(reading)
				validReadings++
			}
			// Sleep to ensure the chip is ready for the next reading
			time.Sleep(20 * time.Millisecond)
		}
	}

	return int(total / int64(samplesToTake)), nil
}
...
// Call after zeroing. Provide the float weight sitting on scale in kg.
